...
function aStarFindPathByGrid(
  ...
  while (toSearch.length) {
    ...
    for (const neighbor of tmpRes) {
      ...
      const costToNeighbor = current.G + current.getManhattanDistanceTo(neighbor);
...
function findNeighborsByGridStep(cur: NodeBase, step: number, bboxes: any, outside:BBoxType) {
  ...
  function isValid(cur: NodeBase | Point, neighbor: NodeBase | Point, bboxes: BBoxType[]) {
    let flag = !isPointInsideTheBoxes(neighbor, bboxes)
...
function perpendicularDistance(point: NodeBase, lineStart: NodeBase, lineEnd: NodeBase) {
