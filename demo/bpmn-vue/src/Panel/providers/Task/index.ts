...
import { TaskNodeHOF } from './task';
import { SubProcessHOF } from './subProcess';
...
export function registerTaskNodes(lf: LogicFlow) {
  const TaskNodeFactory = TaskNodeHOF(lf);

  const ServiceTask = TaskNodeFactory('serviceTask', serviceTaskIcon);
  const UserTask = TaskNodeFactory('userTask', userTaskIcon);

  lf.register(ServiceTask);
  lf.register(UserTask);
  lf.register(SubProcessHOF(lf));
